...
After the migrations has been published you can create the tables on you database by running the migrations:
```bash
php artisan migrate
```
If you change the table names on migrations, please
publish the config and update the tables array.
...
When published, [the `config/acl.php` config file](https://github.com/mateusjunges/laravel-acl/blob/master/config/acl.php) contains:
...
To do this you must update the `models.group` and `models.permission` values in the configuration file after publishing the configuration with this command:
